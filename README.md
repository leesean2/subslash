...
> [!NOTE]
> **계정에는 아직 구독 기록이 저장되지 않습니다.** 계정이 보관하는 것은 아이디·이메일·
> 비밀번호 해시, 이메일 확인 여부, '내 정보'의 선택 항목(나이·성별)뿐입니다. 로그인해도
> 다른 기기에서 같은 구독 목록이 보이지 않으며, 기기를 옮기려면 '내 구독' 아래 **데이터
> 백업**으로 파일을 저장해 새 기기에서 복원해야 합니다. 계정 간 동기화는 아직 없는
> 기능입니다.
